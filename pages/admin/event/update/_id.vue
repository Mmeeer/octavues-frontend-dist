<template>
    <v-layout wrap>
            ...
            <v-flex xs12>
                <v-btn color="success" :disabled="loading" @click="update()">submit</v-btn>
            </v-flex>
    </v-layout>
</template>
...
export default {
  ...
  async asyncData({$axios, params}){
    let res = await $axios.$get(process.env.apiUrl+"/event/update/" + params.id);
    if(res.success){
      let data = {
  ...
  methods: {
    async update(){
      ...
      let res = await this.$axios.$post(process.env.apiUrl+"/event/update/" + this.$route.params.id, data);
      if(res.success){
