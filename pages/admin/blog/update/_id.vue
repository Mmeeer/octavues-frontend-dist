...
export default {
  ...
  async asyncData({$axios, params}){
    ...
    let res = await $axios.$get(process.env.apiUrl+"/blog/update/" + params.id);
    if(res.success){
      let data = {
  ...
  methods:{
    async submit(){
      ...
      let res = await this.$axios.$post(process.env.apiUrl+"/blog/update/" + this.$route.params.id, data)
      this.header = "";
      this.title = "";
      this.content = [];
      this.loading = false;
      if(res.success) this.$router.push("/admin/blog/update/")
    }
  },
  mounted(){
    this.$nextTick(() => {
